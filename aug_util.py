...
def draw_bboxes(img,boxes,predict=[],intersect=[]):
    """
    ...
    source1 = Image.fromarray(img)
    draw1 = ImageDraw.Draw(source1)
    source2 = Image.fromarray(img)
    ...
    for i in range(boxes.shape[0]):
        ...
        for j in range(3):
            draw1.rectangle(((xmin+j, ymin+j), (xmax+j, ymax+j)), fill=(255,0,0,1), outline='black')
    source1 = Image.blend(source1, source, 0.5)
    for i in range(len(predict)):
        if float(predict[i][5]) > 0.5:
            xmin = int(predict[i][0])
            ymin = int(predict[i][1])
            xmax = int(predict[i][2])
            ymax = int(predict[i][3])
            for j in range(3):
                draw2.rectangle(((xmin+j, ymin+j), (xmax+j, ymax+j)), fill=(0,255,0,1), outline='black')
    for i in range(len(intersect)):
        xmin = int(intersect[i][0])
        ymin = int(intersect[i][1])
        xmax = int(intersect[i][2])
        ymax = int(intersect[i][3])
        #for j in range(3):
            #draw2.rectangle(((xmin+j, ymin+j), (xmax+j, ymax+j)), fill=(255,255,0,1))
    source2 = Image.blend(source2, source, 0.5)
    source = Image.blend(source1, source2, 0.5)
    return source
...
def draw_groundtruth(img,boxes):
    ...
    for i in range(boxes.shape[0]):
        ...
        for j in range(3):
            draw1.rectangle(((xmin+j, ymin+j), (xmax+j, ymax+j)), fill=(240,10,10,1), outline='black')
    source1 = Image.blend(source1, source, 0.4)
    return source1
...
def draw_predict(img,predict=[]):
    ...
    for i in range(len(predict)):
        if float(predict[i][5]) > 0.5:
            ...
            ymax = int(predict[i][3])
            for j in range(3):
                draw2.rectangle(((xmin+j, ymin+j), (xmax+j, ymax+j)), fill=(10,240,10,1), outline='black')
    source2 = Image.blend(source2, source, 0.4)
    return source2
